"""
DDDS - Complexity Classifier Fine-Tuning Pipeline
==================================================
...
Expects a CSV with at minimum two columns:
    ...
    label  : int  - 0 (simple) or 1 (complex)
...
def compute_metrics(eval_pred):
    ...
    return {
        ...
        "f1_simple":          f1_cls[0],
        "f1_complex":         f1_cls[1],
        "precision_simple":   p_cls[0],
        "precision_complex":  p_cls[1],
        "recall_simple":      r_cls[0],
        "recall_complex":     r_cls[1],
    }
...
def print_evaluation_report(model, dataset, tokenizer, split_name: str):
    ...
    print(classification_report(
        ...
        target_names=["SIMPLE (0)", "COMPLEX (1)"],
        digits=4
    ))
    ...
    print(f"  True SIMPLE      {cm[0][0]:>6}        {cm[0][1]:>6}")
    print(f"  True COMPLEX     {cm[1][0]:>6}        {cm[1][1]:>6}")


def main(train_csv: str, val_csv: str, test_csv: str | None):
    set_seed(SEED)
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"\n[DDDS] Complexity Classifier Fine-Tuning")
    ...
    print(f"  Train: {len(train_df)} rows  "
          f"(complex: {train_df['label'].sum()}, simple: {(train_df['label']==0).sum()})")
    print(f"  Val:   {len(val_df)} rows  "
          f"(complex: {val_df['label'].sum()}, simple: {(val_df['label']==0).sum()})")

    if test_csv:
        test_df = load_data(test_csv)
        print(f"  Test:  {len(test_df)} rows  "
              f"(complex: {test_df['label'].sum()}, simple: {(test_df['label']==0).sum()})")
    ...
    print(f"  Class weights: SIMPLE={class_weights[0]:.4f}, COMPLEX={class_weights[1]:.4f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fine-tune FinBERT complexity classifier")
